...
from QuartoRL.RL_functions import PHASE_SELECT
...
# Nf_balanceSelect (2): fine-tune from ME(2) E_5000.
# Direct attack on confirmed root cause — class imbalance.
# Adds a separate winner_select_buffer; each batch is assembled so that
# winner-select and loser-select samples are equal (1:1) among select rows.
STARTING_NET = (
    "CHECKPOINTS/ME_endgame(2)0429_ENDGAME_FRACTION_0.5/"
    "20260507_0829-ME_endgame(2)0429_ENDGAME_FRACTION_0.5_E_5000.pt"
)
EXPERIMENT_NAME = "Nf_balanceSelect(2)0507_finetune_ME2"
...
# Replicate ME(2) curriculum: N=2 → N=4, endgame anchor at 50%
...
# Winner-select buffer: stores only transitions where phase==SELECT and outcome==+1.
# Sized to hold 8 epochs of winner-select transitions at N=4
# (worst case: ~1 winner-select per match → 32 per epoch → 256 for 8 epochs).
WINNER_SELECT_BUFFER_SIZE = NUM_EPOCHs_BUFFER * MATCHES_PER_EPOCH * 2
...
MAX_GRAD_NORM = 1.0
LR = 7e-4
LR_F = LR  # constant LR (no cosine decay)
TAU = 0.01
GAMMA = 0.99
...
logger.info(f"WINNER_SELECT_BUFFER_SIZE={WINNER_SELECT_BUFFER_SIZE}")
...
# Balanced select buffer: winner-select transitions only (phase==SELECT, outcome==+1).
# Sampled at equal rate to loser-select transitions in each batch to achieve 1:1 balance.
winner_select_buffer = ReplayBuffer(
    storage=LazyTensorStorage(max_size=WINNER_SELECT_BUFFER_SIZE),
    sampler=SamplerWithoutReplacement(),
)
...
for e in tqdm(
    ...
    if GEN_EXPERIENCE_BY_EPOCH or e == 0:
        ...
        # Extract winner-select transitions: phase==SELECT and outcome==+1.
        # These are stored in a separate buffer to correct the class imbalance.
        sel_win_mask = (exp["phase"] == PHASE_SELECT) & (exp["outcome"] > 0)
        if sel_win_mask.any():
            winner_select_exp = exp[sel_win_mask]
            winner_select_buffer.extend(winner_select_exp)  # type: ignore
            logger.info(
                f"Added {sel_win_mask.sum().item()} winner-select transitions "
                f"(buffer size: {len(winner_select_buffer)})"
            )
    ...
    for i in range(iter_per_epoch):
        # Batch assembly targeting 1:1 winner/loser balance for select samples.
        # Per BATCH_SIZE=32: 16 place + 8 winner-select + 8 loser-select (from regular).
        # Falls back to the standard ME(2) endgame-blend if winner_select_buffer is empty.
        if len(winner_select_buffer) >= 4:
            # 25% of batch = winner-select; 25% = regular select+place (for loser-select);
            # remaining 50% = endgame anchor if available, else regular curriculum.
            winner_sel_size = max(1, BATCH_SIZE // 4)
            if ENDGAME_FRACTION > 0 and len(endgame_replay_buffer) > 0:
                endgame_size = max(1, round(BATCH_SIZE * ENDGAME_FRACTION))
                curriculum_size = BATCH_SIZE - endgame_size - winner_sel_size
                ...
                curriculum_size = BATCH_SIZE - winner_sel_size
                endgame_batch = None
                curriculum_batch = replay_buffer.sample(
                    min(curriculum_size, len(replay_buffer))
                )

            winner_sel_batch = winner_select_buffer.sample(
                min(winner_sel_size, len(winner_select_buffer))
            )

            parts = [curriculum_batch, winner_sel_batch]
            if endgame_batch is not None:
                parts.insert(1, endgame_batch)
            exp_batch = torch.cat(parts, dim=0)
        elif ENDGAME_FRACTION > 0 and len(endgame_replay_buffer) > 0:
            endgame_size = max(1, round(BATCH_SIZE * ENDGAME_FRACTION))
