"""Pa_frozenTrunkSelect(1)0511_N_3 — load ME(2), freeze the ENTIRE trunk
(and fc2_place), train only fc2_select.

Rationale (see Research-status.md → Pa_frozenTrunkSelect):
The Ne_freezePlace fallback ("if WR drops below 40%, also requires_grad=False
on trunk") was never run. Pa is exactly that experiment.

  - WR floor = ME(2)'s 73% by construction (Q_place output is byte-identical).
  - Tests: given the existing trunk features ME(2) learned, can a fresh
    fc2_select find a useful signed mapping?
  - Decision gate: Q_select Δ_pos-neg > 0.40 AND WR vs bot_random ≥ 90%.
  - Cheap: only fc2_select (Linear 128 → 16) is updated. 1000 epochs is plenty.

Architecture / schema / target stack: same as ME(2)
(QuartoCNNAutoreg + decoupled_autoreg + td_place_mc_select + final reward).
"""
...
EXPERIMENT_NAME = "Pa_frozenTrunkSelect(1)0511"
...
EPOCHS = 1_000  # cheap diagnostic; Q_select either lights up within 1k or not

# N=3 fixed: the minimum N that supplies any winner-select transitions
N_LAST_STATES_INIT = 3
N_LAST_STATES_FINAL = 3

MATCHES_PER_EPOCH = 32
NUM_EPOCHs_BUFFER = 8

# Replicate ME(2)'s data regime so fc2_select sees the same distribution that
# trained the rest of the network.
...
logger.info("Successfully loaded ME(2) checkpoint")

target_net.load_state_dict(policy_net.state_dict())

# ──────────────────────────────────────────────────────────────────
# Freeze EVERYTHING except fc2_select.
# This is the missing Ne fallback experiment: full trunk freeze prevents the
# trunk drift that broke Q_place in Ne_freezePlace (WR collapse 73 → 28).
# ──────────────────────────────────────────────────────────────────
trainable_param_names: list[str] = []
for name, p in policy_net.named_parameters():
    if name.startswith("fc2_select"):
        p.requires_grad = True
        trainable_param_names.append(name)
    else:
        p.requires_grad = False
logger.info(
    f"Freeze policy: trainable = {trainable_param_names}; everything else frozen."
)

# Sanity check: only fc2_select.{weight,bias} should train. Fail loudly if not.
expected_trainable = {"fc2_select.weight", "fc2_select.bias"}
actual_trainable = {
    name for name, p in policy_net.named_parameters() if p.requires_grad
}
if actual_trainable != expected_trainable:
    raise RuntimeError(
        "Pa freeze invariant violated. "
        f"Expected trainable={sorted(expected_trainable)}, "
        f"got trainable={sorted(actual_trainable)}."
    )
...
# Optimizer only sees fc2_select. AdamW will refuse empty param groups, so
# guard against the freeze sanity-check passing but the optimizer still being
# misconstructed.
optimizer = optim.AdamW(
    [p for p in policy_net.parameters() if p.requires_grad],
    lr=LR,
    amsgrad=True,
)
assert len(optimizer.param_groups[0]["params"]) == len(
    expected_trainable
), "Optimizer received the wrong number of trainable tensors."
scheduler = None  # no LR schedule for a 1k-epoch diagnostic
...
for e in tqdm(
    ...
    for i in range(iter_per_epoch):
        ...
        if not active_losses:
            raise ValueError("Decoupled batch produced no active place/select samples.")
