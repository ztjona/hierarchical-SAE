...
EXPERIMENT_NAME = "Yc_nStates(4)0616_N_16"
...
#   "td_place_minimax_select" -> place TD/Bellman, select supervised by minimax oracle
#                                (T-series — see USE_MINIMAX_SELECT_TARGET below)
...
# When True, every SELECT transition captured during gen_experience is
# labelled with per-piece minimax scores (depth MINIMAX_SELECT_DEPTH). The
# scores are frozen in the buffer at capture time — the policy of that moment.
# The Q_select loss switches to masked SmoothL1 on the full 16-d Q_select
# vector against the oracle's per-piece target (illegal pieces masked out).
# Q_place training is unchanged.
#
...
# ###########################
# X-series competence levers (default OFF — see docs/diary/series-X.md and
# analysis/competence_audit/REPORT.md). Each adds an auxiliary *ranking* hinge
# (no Q-value-scale coupling) computed in-loop against a frozen 1-ply target
# baked into the replay buffer by gen_experience. unified_autoreg only.
#   X(1) place-win : argmax Q_place should land on an immediate winning cell.
...
# lever is active; otherwise the buffer keys are emitted as zeros (no-op).
# target_hot_piece is shared by the select-margin hinge (X3) and the aux hot head.
EMIT_PLACE_WIN = LAMBDA_PLACE_WIN > 0.0
EMIT_HOT_MASK = (LAMBDA_SEL_MARGIN > 0.0) or (LAMBDA_HOT > 0.0)
...
logger.info(
    f"X-levers: LAMBDA_PLACE_WIN={LAMBDA_PLACE_WIN}, "
    f"LAMBDA_SEL_MARGIN={LAMBDA_SEL_MARGIN}, LAMBDA_HOT={LAMBDA_HOT}, "
    f"WIN_MARGIN={WIN_MARGIN} "
    f"(EMIT_PLACE_WIN={EMIT_PLACE_WIN}, EMIT_HOT_MASK={EMIT_HOT_MASK})"
)
...
loss_data: dict[str, list[float | int]] = {
    ...
    "loss_hot_values": [],  # LAMBDA_HOT * BCE(hot_logits, target_hot_piece), weighted (NaN if off this step)
...
for e in tqdm(
    ...
    for i in range(iter_per_epoch):
        ...
        if TRANSITION_SCHEMA in ("decoupled_autoreg", "unified_autoreg"):
            # Minimax-select target style returns a 5-tuple with a (n_sel, 16)
            # legality mask; all other styles return the original 4-tuple.
            ...
            if sel_mask is not None:
                # Masked SmoothL1 on the full 16-d Q_select vector against the
                # oracle target. Illegal pieces (mask=0) contribute zero loss.
        ...
        # X-series competence levers (no-op unless a LAMBDA_* is > 0). Adds an
        # auxiliary ranking hinge on the policy net's own head outputs against
        # frozen 1-ply targets in the buffer; see win_margin_aux_loss.
        if LAMBDA_PLACE_WIN > 0.0 or LAMBDA_SEL_MARGIN > 0.0:
            loss = loss + win_margin_aux_loss(
                policy_net,
                exp_batch,  # type: ignore
                lambda_place_win=LAMBDA_PLACE_WIN,
                lambda_sel_margin=LAMBDA_SEL_MARGIN,
                margin=WIN_MARGIN,
            )

        # Stage-A aux hot-piece head: a dense per-piece BCE that forces the
        # shared trunk to encode select-safety (no-op unless LAMBDA_HOT > 0 and
        # the model has a hot head). Pure training-time scaffold — fc_hot is not
        # used at inference. See docs/diary/series-X.md.
        L_hot = None
        if LAMBDA_HOT > 0.0:
            L_hot = hot_head_aux_loss(
                policy_net,
                exp_batch,  # type: ignore
        ...
        loss_data["loss_hot_values"].append(
            float(L_hot.item()) if L_hot is not None else float("nan")
        )
