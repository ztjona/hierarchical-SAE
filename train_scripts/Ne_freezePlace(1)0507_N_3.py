...
# Ne_freezePlace: load ME(2) checkpoint, freeze fc2_place, train fc2_select + trunk.
# Hypothesis: Q_select gradient is starved by the dominant Q_place gradient.
# Freezing fc2_place removes the dominant gradient source and forces the trunk
# to adapt to select-only updates.
STARTING_NET = (
    "CHECKPOINTS/ME_endgame(2)0429_ENDGAME_FRACTION_0.5/"
    "20260507_0829-ME_endgame(2)0429_ENDGAME_FRACTION_0.5_E_5000.pt"
)
EXPERIMENT_NAME = "Ne_freezePlace(1)0507_N_3"
...
# N=3 fixed — minimum for winner-select transitions; no curriculum
...
# No endgame buffer — this experiment must let winner-select transitions compete fairly.
ENDGAME_FRACTION = 0
...
MAX_GRAD_NORM = 1.0
LR = 7e-4        # trunk + fc_in_aux + phase_embedding
LR_SELECT = 5e-3  # 7× select head (from Ne spec)
LR_F = LR        # constant (no cosine decay)
TAU = 0.01
GAMMA = 0.99

# From Ne onward: ME(2) replaces bot_loss-BT as the primary benchmark.
BASELINES = [
    {
        "path": (
            "CHECKPOINTS/ME_endgame(2)0429_ENDGAME_FRACTION_0.5/"
...
logger.info(
    f"Train conf.:\t{EPOCHS=}, {BATCH_SIZE=}, {LR=}, {LR_SELECT=}, {LR_F=}, {GAMMA=}, {TAU=}, {MAX_GRAD_NORM=}"
)
...
# Freeze fc2_place: gradient-starvation diagnostic.
# fc2_place parameters are excluded from the optimizer entirely so their weights
# stay fixed at the ME(2) values throughout training.
for p in policy_net.fc2_place.parameters():
    p.requires_grad = False
logger.info("fc2_place frozen (requires_grad=False)")
...
# Asymmetric optimizer: fc2_select at 7× LR; trunk + fc_in_aux + phase_embedding
# at base LR. fc2_place is excluded entirely (frozen).
fc2_place_param_ids = {id(p) for p in policy_net.fc2_place.parameters()}
select_param_ids = {id(p) for p in policy_net.fc2_select.parameters()}
trainable_params = [
    p for p in policy_net.parameters() if p.requires_grad
]
trunk_params = [
    p for p in trainable_params if id(p) not in select_param_ids
]
...
scheduler = None  # constant LR throughout
...
for e in tqdm(
    ...
    logger.info(f"LR trunk={LR}, LR_select={LR_SELECT} (constant, no schedule)")
