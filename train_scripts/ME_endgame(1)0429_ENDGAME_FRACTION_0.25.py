...
EXPERIMENT_NAME = "ME_endgame(1)0429_ENDGAME_FRACTION_0.25"
CHECKPOINT_FOLDER = f"./CHECKPOINTS/{EXPERIMENT_NAME}/"
# Autoreg schema: shared trunk + phase embedding, place/select heads called separately
TRANSITION_SCHEMA = "decoupled_autoreg"
ARCHITECTURE = QuartoCNNAutoreg
PLAYER_BOT_CLASS = Quarto_autoreg_bot

# TD for Q_place (Bellman), Monte Carlo for Q_select
...
# Curriculum: start at N=2 (end-game only), linearly expand to N=4
...
# Fraction of each training batch drawn from the end-game anchor buffer (N=2).
# This prevents catastrophic forgetting as the curriculum buffer expands to N>2.
ENDGAME_FRACTION = 0.25
N_LAST_STATES_ENDGAME = 2

# STEPS_PER_EPOCH uses N_LAST_STATES_FINAL for buffer sizing (worst-case horizon)
STEPS_PER_EPOCH = (
...
FREQ_EPOCH_PLOT_SHOW = 1_000_000  # effectively disable mid-run plots
