...
# Ng_auxSelect: 1-step Q_place_target bootstrap as the supervised target for Q_select.
# For each select transition: target_select = γ * max_a Q_place_target(s').
# No Q_select self-bootstrap, no MC variance, no outcome-label imbalance.
# Diagnostic: load ME(2), N=3 fixed, 2000 epochs.
STARTING_NET = (
    "CHECKPOINTS/ME_endgame(2)0429_ENDGAME_FRACTION_0.5/"
    "20260507_0829-ME_endgame(2)0429_ENDGAME_FRACTION_0.5_E_5000.pt"
)
EXPERIMENT_NAME = "Ng_auxSelect(1)0507_N_3"
...
# Key change: select head gets 1-step Q_place_target bootstrap instead of MC return.
DECOUPLED_TARGET_STYLE = DECOUPLED_TARGET_TD_PLACE_TD_SELECT
