...
from models.CNN_autoreg import QuartoCNNAutoreg, QuartoCNNAutoregUnbound, QuartoCNNAutoregSepTrunks
...
# Nh_sepTrunks: separate convolutional trunks for place and select heads.
# Tests shared-trunk gradient interference as an isolated hypothesis.
# Train from scratch with the ME(2) curriculum (N=2→4, ENDGAME_FRACTION=0.5).
STARTING_NET = None  # train from scratch
EXPERIMENT_NAME = "Nh_sepTrunks(1)0507_scratch"
CHECKPOINT_FOLDER = f"./CHECKPOINTS/{EXPERIMENT_NAME}/"
TRANSITION_SCHEMA = "decoupled_autoreg"
ARCHITECTURE = QuartoCNNAutoregSepTrunks  # separate trunks for each head
...
logger.info("Starting with random weights (no checkpoint — train from scratch)")
