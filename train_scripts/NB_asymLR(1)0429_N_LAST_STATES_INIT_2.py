...
STARTING_NET = None
EXPERIMENT_NAME = "NB_asymLR(1)0429_N_LAST_STATES_INIT_2"
CHECKPOINT_FOLDER = f"./CHECKPOINTS/{EXPERIMENT_NAME}/"
TRANSITION_SCHEMA = "decoupled_autoreg"
# Same bounded autoreg model as MB — the only change is the optimizer configuration below.
...
MAX_GRAD_NORM = 1.0
# Asymmetric learning rates: fc2_select gets 10× the base LR so the select head
# can learn against the ~280× stronger Q_place trunk gradient. All other params
# (trunk + fc2_place) keep the standard MB LR.
LR = 7e-4        # base LR — trunk, fc2_place, fc_in_aux, conv1, conv2, fc1
LR_SELECT = 7e-3  # 10× for the select output head only
LR_F = LR        # no cosine decay (constant throughout, same as MB)
TAU = 0.01
GAMMA = 0.99
...
# Asymmetric optimizer: fc2_select at 10× LR, all other parameters at base LR.
# This directly compensates the gradient-starvation imbalance without changing
# the model architecture or the loss formulation.
select_param_ids = {id(p) for p in policy_net.fc2_select.parameters()}
optimizer = optim.AdamW(
    [
        {
            "params": [p for p in policy_net.parameters() if id(p) not in select_param_ids],
            "lr": LR,
        },
        {
            "params": list(policy_net.fc2_select.parameters()),
            "lr": LR_SELECT,
        },
    ],
    amsgrad=True,
)
# No LR schedule: LR_F == LR means CosineAnnealingLR would be a no-op for group 0
# and would anneal group 1 from LR_SELECT down to LR_F — keeping constant rates
# for both groups is simpler and matches the MB baseline behaviour.
scheduler = None
...
for e in tqdm(
    ...
    # No scheduler.step() — constant LRs throughout
    logger.info(f"LR base={LR}, LR_select={LR_SELECT} (constant, no schedule)")
