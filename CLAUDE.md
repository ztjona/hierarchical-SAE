...
- **"Q_select saturation" was largely a metric artefact (2026-05-19, D1 diagnostic).** The match-outcome-conditioned Δ in `QuartoRL/results_io.py` (`q_select_winners` / `q_select_losers`) averages over functionally-neutral states and hides per-position structure that Q_select does encode under oracle supervision. Use the D1 fields shipped in the JSONL (`d1_safe_piece_recall` ≥ 0.70 vs chance ≈ 0.11; `d1_forcing_loss_bottom_recall`; `d1_spearman_rho_mean`) as the select-side gate, not the Δ. Before treating any "Q_select looks flat" finding as load-bearing, read `analysis/qselect_diagnostics/REPORT.md` and `Research-status.md` → `Current Open Problem`.
